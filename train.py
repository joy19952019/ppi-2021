...
# parser.add_argument(
#     "--BC6PM_dir", default='/home/fy/Desktop/ldcode/BC6PM', help = 'the directory of dataset')#add for debug
...
parser.add_argument("--batch_size_rc", default=4, type=int)
...
parser.add_argument("--max_epoch_ner", default=25, type=int)
parser.add_argument("--max_epoch_rc", default=10, type=int)
parser.add_argument("--max_epoch_triage", default=25, type=int)
parser.add_argument("--max_epoch_joint", default=25, type=int)
parser.add_argument("--fold_num", default=10, type=int)#for debugging gauss!
...
def train_RC(model, iterator, dataset, epoch):
    ...
    for i, batch in enumerate(iterator):
        x, labels, guid, P_gauss1_list, P_gauss2_list = batch#
        # print("x: ", x)#debug
        # # print("x.shape: ", x.sizeof())
        # print("labels: ", labels)
        # print("guid:", guid)
        # print("P_gauss1_list:", P_gauss1_list)
        # print("P_gauss2_list:", P_gauss2_list)
        model.optimizer.zero_grad()
        logits, y, y_hat = model.forwardRC(x, labels, P_gauss1_list, P_gauss2_list)
...
def eval_RC(model, iterator, epoch, documents_eval, documents_eval_ground_truth):
    ...
    # print("f1's result is:", f1)
    # print("micro_precision's result is:", micro_precision)
    # print("micro_recall's result is:", micro_recall)
    # print("bc6's result is:", micro_f1)
    # print("countDict is:", countDict)
        ...
        if args.do_train:
            ...
            if args.task.lower() == 'bc2gm':
                ...
                if args.do_cross_valid:
                    if fold_iter_idx == 0:
                        if args.train_withGNP:
                            train_data_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'GNormPlus', 'withAnn-Result', 'PMtask_Relations_TrainingSet_r.json')#os.environ.get('BC6PM_dir')
                        else:
                            train_data_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'json', 'PMtask_Relations_TrainingSet.json')
                        with open(train_data_dir) as f:
                            train_data = json.load(f)
                        documents = train_data['documents']
                        # documents = documents[:10]#for debugging gauss!
                        if args.do_sanity_check:
                    ...
                    # Triage Data
                    if fold_iter_idx == 0:
                        with open(os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'json', 'PMtask_Triage_TrainingSet.json')) as f:
                    ...
                    if args.train_withGNP:
                        train_data_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'GNormPlus', 'withAnn-Result', 'PMtask_Relations_TrainingSet_r.json')
                    else:
                        train_data_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'json', 'PMtask_Relations_TrainingSet.json')
                    with open(train_data_dir) as f:
                        train_data = json.load(f)
                    ...
                    with open(os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'GNormPlus', 'result', 'PMtask_Relations_TestSet_r.json')) as f:
                        eval_data = json.load(f)
                    documents_eval = eval_data['documents']
                    with open(os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'json', 'PMtask_Relations_TestSet.json')) as f:
                    ...
                    with open(os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'json', 'PMtask_Triage_TestSet.json')) as f:
            ...
            # Process Data
            if args.do_ner:
                bio_train_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'BIO-tag', 'train.tsv')
                bio_eval_dir = os.path.join('/home/fy/Desktop/ldcode/BC6PM', 'BIO-tag', 'dev.tsv')
